...
gwxAddEvent(int type, int x, int y, int state)
{
    ...
    printf("gwxaddevent type %d x %d y %d state %d\n",type,x,y,state);  
    event->window = gwxWin;
    event->type   = type;
    event->x      = x;
    event->y      = y;
...
gwxRemEvent(int *type, int *x, int *y, int *state)
{
    ...
    printf("gwxremevent type %d x %d y %d state %d\n",type,x,y,state);  
    event = gwxEvents;
...
gwxWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
 ...
 printf("mousemove active %d focus %d\n",GetActiveWindow(),GetFocus()); 
    ...
    case WM_MBUTTONDOWN:
	printf("mouseMBUTTONDWN %d  %d\n",hWnd,gwxWin);  
        gwxAddEvent(XButtonPress, LOWORD(lParam), HIWORD(lParam), 2);
        return 0;

    case WM_MBUTTONUP:
	printf("mouseMBUTTONUP %d  %d\n",hWnd,gwxWin);  
        gwxAddEvent(XButtonRelease, LOWORD(lParam), HIWORD(lParam), 2);
        return 0;

    case WM_RBUTTONDOWN:
	printf("mouseRBUTTONDWN %d  %d\n",hWnd,gwxWin);  
...
GWXCURS2(int *x, int *y, int *state)
{
  ...
  *y     = point.y - gwxSize[3];
  *state = key;
  printf("gwxcurs2 x %d y %d state %d\n",*x,*y,key);
}
