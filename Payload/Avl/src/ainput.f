...
C
     ...
     &     BRADY(NWRK), BRADZ(NWRK), CLAF(NWRK)
      ...
      REAL CLCDSEC(6,NWRK)
cc#ifdef USE_CPOML
      REAL XLASEC(IBX,NWRK), ZLASEC(IBX,NWRK)
      REAL XUASEC(IBX,NWRK), ZUASEC(IBX,NWRK)
cc#endif
C
      REAL XB(IBX), YB(IBX)
...
C------ assume surface uses full airfoil range
        LRANGE(ISURF) = .TRUE.
...
c...      lower/upper coordinates (for oml)
          ...
          ENDIF
          TH = ATAN(SLP)
          XLASEC(I,ISEC) = XF + 0.5*THK*SIN(TH)
          XUASEC(I,ISEC) = XF - 0.5*THK*SIN(TH)
          ZLASEC(I,ISEC) = ZF - 0.5*THK*COS(TH)
          ZUASEC(I,ISEC) = ZF + 0.5*THK*COS(TH)
cc#endif
        ENDDO
...
cc#ifdef USE_CPOML
C...      lower/upper coordinates (for oml)
          CALL AKIMA(XIN,YIN,NIN,XASEC(I,ISEC),ZC,DUMMY)
          XLASEC(I,ISEC) = XASEC(I,ISEC)
          XUASEC(I,ISEC) = XASEC(I,ISEC)
          ZLASEC(I,ISEC) = ZC - 0.5*TASEC(I,ISEC)
          ZUASEC(I,ISEC) = ZC + 0.5*TASEC(I,ISEC)
cc#endif
        END DO
        CALL NRMLIZ(NASEC(ISEC),XASEC(1,ISEC))
C
